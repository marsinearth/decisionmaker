...
class App extends Component {
    ...
    addInputText= () => {
        var maxIndex = Number(this.state.maxInputIndex);
    ...
    render() {
        const {selectedOption, question, inputs, nums, items, answer} = this.state;
        const maskNumSetup = { display: selectedOption === "numbers" ? 'block' : 'none' };
        const maskCustSetup = { display: selectedOption === "custom" ? 'block' : 'none' };
        const objKeys = selectedOption === "custom" ? Object.keys(items) : Object.keys(nums);
        const inputList = inputs.map(index => <CustomInputText
                                                  name={index}
                                                  key={index}
        ...
        return (
            <div className="App">
	            ...
	            <div className='container'>
	                        ...
	                        <div className="weui_cell" style={{'display':'inline-block'}}>
	                            <div style={{'textAlign':'left'}}>
	                                <input 
	                                    ...
	                                    name="fromNum"
	                                    value={this.state.nums["fromNum"] ? this.state.nums["fromNum"] : ''}
	                                    onChange={this.handleInputChange}
                                    />      
	                            </div>
	                            <div style={{'textAlign':'left'}}>
	                                <input 
	                                    ...
	                                    name="toNum"
	                                    value={this.state.nums["toNum"] ? this.state.nums["toNum"] : ''} 
                                        onChange={this.handleInputChange}
                                    />
	                            </div>
	                        </div>
	                ...
	                <Slotmachine option={selectedOption} nums={nums} items={items} onSubmitClick={this.handleSubmit} disabled={disabled} answer={this.answerOn} />     
	            </div>
            </div>	    
        );
    }
